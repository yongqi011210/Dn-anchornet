...
class DOTADataset(CustomDataset):
    ...
    def evaluate(self,
        ...
        if metric == 'mAP':
            ...
            # 调用 eval_rbbox_map 获取结果
            result = eval_rbbox_map(
                results,
                annotations,
            ...
            if isinstance(result, tuple):
                if len(result) >= 1:
                    mean_ap = result[0]  # 第一个值总是 mAP
                if len(result) >= 2:
                    # 只记录数值类型的结果，不保存复杂对象
                    pass
                if len(result) >= 3:
                    # 提取 FAR 和 FPPI 数值
                    far_metrics = result[2]
                    eval_results['FAR'] = far_metrics.get('far', 0.0)
                    eval_results['FPPI'] = far_metrics.get('fppi', 0.0)
            else:
                mean_ap = result
