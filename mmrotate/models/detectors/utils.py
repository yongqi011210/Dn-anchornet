...
class AlignConv(nn.Module):
    ...
    def get_offset(self, anchors, featmap_size, stride):
        """
        anchors: [N, 5], N = feat_h * feat_w * anchors_per_point (anchors_per_point=1 here)
        featmap_size: (H, W)
        """
        ...
        # 采样点的卷积坐标，大小为 kernel_size*kernel_size
        xc = torch.arange(0, feat_w, device=device, dtype=dtype)
        yc = torch.arange(0, feat_h, device=device, dtype=dtype)
        yc, xc = torch.meshgrid(yc, xc, indexing='ij')
        xc = xc.reshape(-1)
        yc = yc.reshape(-1)
        x_conv = xc[:, None] + xx  # [H*W, kernel_size*kernel_size]
        y_conv = yc[:, None] + yy  # [H*W, kernel_size*kernel_size]
        ...
        offset_x = x_anchor - x_conv  # [H*W, kernel_size*kernel_size]
        offset_y = y_anchor - y_conv

        # deform conv要求offset shape为 [N, 2*kernel_size*kernel_size, H, W]
        offset = torch.stack([offset_y, offset_x], dim=-1)  # [H*W, kernel_size*kernel_size, 2]
        offset = offset.reshape(feat_h * feat_w, -1)  # [H*W, 2*kernel_size*kernel_size]
        offset = offset.permute(1, 0).reshape(2 * self.kernel_size * self.kernel_size, feat_h, feat_w)  # [C, H, W]
    ...
    def forward(self, x, anchors):
        ...
        anchors: list of length N, each element shape [num_anchors, 5]
        """
        N, C, H, W = x.shape
        assert N == len(anchors), f"anchors list length {len(anchors)} must equal batch size {N}"

        offset_list = []
        for i in range(N):
            anchor = anchors[i]
            # 计算offset时，anchors数量应匹配H*W*anchors_per_point
            offset = self.get_offset(anchor, (H, W), self.stride)
            offset_list.append(offset)

        offset_tensor = torch.stack(offset_list, dim=0)  # [N, offset_channels, H, W]
        out = self.relu(self.deform_conv(x, offset_tensor.detach()))
        return out
    # def forward(self, x, anchors):
...
class AlignConvModule(nn.Module):
    ...
    def forward(self, x, rbboxes):
        """
        Args:
            x (list[Tensor]): multi-scale feature maps, length=L, each shape [N, C, H, W]
            rbboxes (list[list[Tensor]]): length=batch_size, each element is list of length=L of anchors tensor [num_anchors, 5]
        """

        # 转置batch和levels维度，得到length=L的列表，每个元素是batch个anchors组成的tuple
        mlvl_rbboxes = list(zip(*rbboxes))

        out = []
        for feat_map, anchors_per_level, ac in zip(x, mlvl_rbboxes, self.ac):
            # anchors_per_level是tuple，转成list传入AlignConv
            anchors_per_level = list(anchors_per_level)
            feat_refined = ac(feat_map, anchors_per_level)
            out.append(feat_refined)
